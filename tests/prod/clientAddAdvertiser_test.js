...
Feature('prod client add advertiser');

Scenario('PROD Тест добавление нового advertiser c пустыми полями', {retries: 2}, async ({I}) => {
...
}).tag('prodClientAddAdvertiser1');

Scenario('PROD Тест добавление нового advertiser c пустым полем Cities of operation', {retries: 2}, async ({I}) => {
    // Создаем нового клиента
    I.amOnPage(basePage.url.prod.clientRegister);
    ...
    I.see("Cities of operation cannot be blank.")
}).tag('prodClientAddAdvertiser2');

Scenario('PROD Тест добавление нового advertiser c пустым полем Name', {retries: 2}, async ({I}) => {
    // Создаем нового клиента
    I.amOnPage(basePage.url.prod.clientRegister);
...
}).tag('prodClientAddAdvertiser3');

Scenario('PROD Тест добавление нового Advertiser', {retries: 2}, async ({I}) => {
    // Создаем нового клиента
    I.amOnPage(basePage.url.prod.clientRegister);
...
}).tag('prodClientAddAdvertiser4');

Scenario('PROD Тест добавление нового Advertiser у старого клиента', {retries: 2}, async ({I}) => {
